...
const Label: React.FC<{ frame: number; from: number; text: string; x: number; y: number; color: string }> = ({ frame, from, text, x, y, color }) => {
  ...
  return (
    <div style={{ position: 'absolute', left: x, top: y, color: COLORS.text, opacity: p, transform: `translateY(${(1 - p) * 24}px)`, zIndex: 20 }}>
      <div style={{ width: 44, height: 4, borderRadius: 99, background: color, marginBottom: 13 }} />
      <div style={{ fontSize: 46, fontWeight: 780, letterSpacing: -1.8 }}>{text}</div>
    </div>
  );
};
...
const FloatingTile: React.FC<{ frame: number }> = ({ frame }) => {
  ...
  return (
    <div style={{ position: 'absolute', left: '50%', top: '50%', width: 285, height: 180, borderRadius: 30, background: 'linear-gradient(135deg, rgba(56,183,255,0.28), rgba(140,108,255,0.18))', border: '1px solid rgba(255,255,255,0.16)', boxShadow: '0 30px 90px rgba(0,0,0,0.48)', transform: `translate(-50%, -50%) translate3d(${x}px, ${y}px, 180px) rotateY(${r * 2}deg) rotateZ(${r}deg) scale(${scale})`, overflow: 'hidden', zIndex: 10 }}>
      <div style={{ position: 'absolute', left: 24, top: 22, width: 68, height: 10, borderRadius: 99, background: COLORS.cyan }} />
      <div style={{ position: 'absolute', left: 24, right: 24, bottom: 28, display: 'flex', gap: 10 }}>
        {[0, 1, 2, 3].map((i) => <div key={i} style={{ flex: 1, height: 40 + i * 12, borderRadius: 12, background: 'rgba(255,255,255,0.16)' }} />)}
      </div>
    </div>
  );
};
  ...
  return (
    <SceneShell vignette={0.58}>
      ...
      <div style={{ position: 'absolute', inset: 0, perspective: 1240, transformStyle: 'preserve-3d' }}>
        <div style={{ transform: `scale(${1 + cutEnergy * 0.032}) rotateZ(${cutEnergy * -0.25}deg)`, transformStyle: 'preserve-3d' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: `rgba(255,255,255,${cutEnergy * 0.07})`, mixBlendMode: 'screen' }} />
        <div
          style={{
            position: 'absolute',
            left: 142,
